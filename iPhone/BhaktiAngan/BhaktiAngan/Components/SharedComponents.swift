...
enum LegalCopy {
    ...
    Purchases are processed by Apple through the App Store. Notification permission is used only for the daily darshan reminder you choose to enable. Photo-library add permission is used only when you tap Save to keep a wallpaper. Location permission, if you grant it, is used only on your device to calculate Panchang timings (sunrise, sunset, and auspicious periods) and is never transmitted or stored — you can instead pick a city by hand.

    Because your data stays on your device, removing the app removes the data. The in-app Contact Support form opens your own mail app to write to us; it does not send anything to our servers automatically. For any question about this policy, use Contact Support in Settings or the support link on our App Store listing.
    """
    ...
    City names, coordinates, and time zones used for Panchang calculations are derived from the GeoNames geographical database, © GeoNames, licensed under Creative Commons Attribution 4.0 (CC BY 4.0). See https://www.geonames.org and https://creativecommons.org/licenses/by/4.0/.
